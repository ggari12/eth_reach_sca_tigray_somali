...
df_education_samples <- readxl::read_excel("../support_files/sca_sample_size.xlsx", sheet = "education") |> 
    select(woreda, facility_assessed, required_samples = sample_size)

df_health_samples <- readxl::read_excel("../support_files/sca_sample_size.xlsx", sheet = "health") |> 
    select(woreda, facility_assessed, required_samples = sample_size)

df_water_points_samples <- readxl::read_excel("../support_files/sca_sample_size.xlsx", sheet = "water_points") |> 
    select(woreda, facility_assessed, required_samples = sample_size)
...
>Total number of collected surveys: **`r nrow(df_updated_tool_data)`**,\
...
df_updated_tool_data |> 
  filter(facility_assessed == "educaation") |> 
  group_by(woreda) |> 
  ...
  relocate(c(region, zone, woreda, woreda), .before = number_of_surveys) |>
  dt_options_fewcols()
```
...
### Water points Facility: **`r df_updated_tool_data %>% filter(facility_assessed == "water_points") %>% nrow()`** surveys

```{r, echo = FALSE}
df_samp_per_location <- df_water_points_samples
df_updated_tool_data |> 
  filter(facility_assessed == "water_points") |> 
  group_by(woreda) |> 
  summarise(number_of_surveys = n()) |> 
  arrange(woreda) |> 
  full_join(df_samp_per_location, by = c("woreda" = "woreda")) |>
